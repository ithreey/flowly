...
use hyper::{Body, HeaderMap, Request, Response, StatusCode, body::*, header};
use log::error;
...
/// 判断 Content-Type 是否属于可安全按纯文本处理并替换的类型（text/*、JSON、JavaScript）。
///
/// 忽略 `;` 后的参数部分（如 `charset`），media type 统一转小写后匹配。
pub fn is_textual_content_type(content_type: &HeaderValue) -> bool {
        ...
        match self {
            ...
            Modify::Body(bm) => {
                ...
                if match parts.headers.get(header::CONTENT_TYPE) {
                    Some(content_type) => is_textual_content_type(content_type),
                    None => false,
                } {
