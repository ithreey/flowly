use crate::{
    ...
    sni_reader::{
        HandshakeRecordReader, PrefixedReaderWriter, RecordingBufReader,
        read_sni_host_name_from_client_hello,
    },
};
use http::{HeaderValue, Uri, header, uri::Scheme};
use hyper::{
    Body, Method, Request, Response, body::HttpBody, server::conn::Http, service::service_fn,
};
use log::*;
    ...
    D: CustomContextData,
{
    ...
    pub async fn serve_tls<IO: AsyncRead + AsyncWrite + Unpin + Send + 'static>(
        ...
        pin!(reader);
        // 非 TLS 连接 / 超时 / 解析失败时直接关闭连接，避免 panic。
    ...
    fn get_cert_res(&self) -> hyper::Response<Body> {
        Response::builder()
            .header(
                ...
                "attachment; filename=Flowly.crt",
            )
