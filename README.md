...
3. 启动 Flowly 代理。
4. 将本机浏览器或客户端的 HTTP/HTTPS 代理设置为 `127.0.0.1:34567`；局域网设备请使用运行 Flowly 机器的局域网 IP，例如 `192.168.1.10:34567`。
5. 在流量监控页面查看会话。

桌面端页面只需要配置端口。应用内部会监听 `0.0.0.0:<端口>`，本机系统代理会指向 `127.0.0.1:<端口>`，局域网设备使用运行 Flowly 机器的局域网 IP 和相同端口连接。
